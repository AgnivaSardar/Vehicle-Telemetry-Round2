...
export default function ViewTelemetry() {
  ...
  const stats = useMemo((): TelemetryStats => {
    ...
    const avgSpeed =
      telemetryData.reduce((sum, d) => sum + d.speed, 0) /
      telemetryData.length;

    const avgTemp =
      telemetryData.reduce((sum, d) => sum + d.temperature, 0) /
      telemetryData.length;

    const avgBattery =
      telemetryData.reduce((sum, d) => sum + d.battery, 0) /
      telemetryData.length;
